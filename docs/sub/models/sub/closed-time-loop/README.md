...
```text
find earliest F where:
state(loop-compatible, F) != state(loop-free, F)
...
-> invention of time-travel mechanism
-> later D
-> isolated past-directed transit
-> same R
```
...
-> earlier decision changes
-> consequences
-> later event supplying the information
...
The model should be compared with established or existing work on:
